...
namespace TP3.Controladores
{
    class nPartida
    {
        ...
        public static List<Usuario> ListarPorPuntos()
        {
            ...
            {
                Console.WriteLine("{0} - Nombre: {1}  Puntos: {2}", i+1, u.Nombre, u.Puntos);
                i += 1;
            }
            return usuarios;
        }
        ...
        public static void imprimirUsuariosxTema()
        {
            ...
            Tema t = pTema.GetTemaById(n);
            Console.Clear();
            Console.WriteLine("ID:  " + t.Id + ", Tema: " + t.Nombre);
            for (int i = 0; i < RankingXTema(n).Count; i++)
            {
                Console.WriteLine((i + 1) + "- Nombre: " + nPartida.RankingXTema(n)[i]);
            }
        }

        public static void ListarTemas() 
        { 
            foreach(Tema t in pTema.GetAll())
            {
                Console.WriteLine("ID: {0}, Nombre: {1}", t.Id, t.Nombre);
            }
        }
    }
}